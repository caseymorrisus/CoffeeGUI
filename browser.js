...
function watchDirectory (bool) {
	if (bool) {
		...
		gaze = null;
		gaze = new Gaze('*.coffee', {cwd: options.watch.directory}, function (err, watcher) {
			this.on('changed', function (filepath) {
				console.log(filepath + ' was changed');
				var dest = getDestination(filepath);
				start = +new Date();
				readCoffeeFile(filepath, dest, compileCoffee);
			});
...
holder.ondrop = function (e) {
	...
	if ( files.length === 1 ) {
		...
		if ( extension === '.coffee' ) {
			readCoffeeFile(file.path, dest, compileCoffee);
		} else {
		...
		_.forEach(files, function(n, key) {
			if ( path.extname(files[key].path) === '.coffee' ) {
				coffeeFiles.push(files[key].path);
			}
		});
		readManyCoffeeFiles(coffeeFiles, dest, compileCoffee);
	};
	return false;
};
...
function compileCoffee (coffeeStr, fileName, dest , many) {
	var js = coffee.compile(coffeeStr);
...
function readCoffeeFile (src, dest, callback) {
	var fileName = path.basename(src, '.coffee');
...
function readManyCoffeeFiles (array, dest, callback) {
